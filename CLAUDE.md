...
- **MCP: один файл = одна семья тулов.** `mcp/tools/*_tools.py` экспортируют
  ...
  **фильтрует уже зарегистрированный** каталог (`mcp/profiles.py`). Профиль
  ...
  самодостаточный payload; дальше `minimal` 20 / `standard` 116 / `full` 120.
...
| `test_server_profiles.py` | counts профилей (6/20/116/120) в коде и доках совпадают |
...
- MCP-сервер `cod-doc` (native stdio, `.mcp.json` явно ставит профиль
  `standard`, не дефолтный `agent`) — 116 тулов `task_*`/`doc_*`/`plan_*`/…;
