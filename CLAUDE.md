...
```bash
scripts/gate.sh                          # все четыре шага; зелёный — одна строка
```

Раскрывается ровно в то, что гоняет CI:
...
Звать раннер, а не четыре команды руками: зелёный прогон иначе печатает ~2400
точек прогресса, сводку warnings и четыре «ок» — информация с нулевой
энтропией, которая занимает и терминал, и контекст агента. Раннер держит полный
вывод в лог-файле, на stdout отдаёт одну строку, а при падении — выжимку по
каждому упавшему шагу (полосу прогресса и warnings из неё вырезает) и путь к
логу. Гоняет все четыре шага даже после раннего падения: один прогон должен
давать полную картину. Код возврата — число упавших шагов.

**Зелёный гейт не результат и в отчёт не идёт** — ни числа тестов, ни «гейт
зелёный». Сообщается только красное. Зелёные тесты говорят одно: старое
поведение не сломано; доказательством правильности нового они не являются.
