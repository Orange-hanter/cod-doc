...
- **MCP: один файл = одна семья тулов.** `mcp/tools/*_tools.py` экспортируют
  ...
  **фильтрует уже зарегистрированный** каталог (`mcp/profiles.py`). Профиль
  ...
  самодостаточный payload; дальше `minimal` 21 / `standard` 111 / `full` 115.
...
| `test_server_profiles.py` | counts профилей (6/21/111/115) в коде и доках совпадают |
...
- MCP-сервер `cod-doc` (native stdio, `.mcp.json` явно ставит профиль
  `standard`, не дефолтный `agent`) — 111 тулов `task_*`/`doc_*`/`plan_*`/…;
