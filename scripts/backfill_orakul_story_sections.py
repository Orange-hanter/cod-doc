...
"""ADO-143: разложить истории orakul по секциям из исходного markdown.
    ...
    ONE-SHOT, ADO-143, НЕ ВЫПОЛНЕН на 2026-09-18.

Скрипт одноразовый и пишет в БД ЧУЖОГО проекта (orakul), а не cod-doc,
поэтому в рамках PR по секциям историй он не запускался: это решение
владельца orakul, а не побочный эффект мержа. По умолчанию — сухой
прогон; запись делает только `--apply`.

Тестом сознательно не покрыт: одноразовый скрипт, у которого есть тест,
живёт вечно. Когда бэкфилл выполнят — заменить строку выше на
`ONE-SHOT, ADO-143, executed <дата>`, и файл можно удалять.
