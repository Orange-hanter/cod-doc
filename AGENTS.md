...
> + `cod_doc/mcp/tools/curator_tools.py`. Счётчики 6/21/140/144.
...
9. **MCP server profiles** (PCA-951, cycle-4 default-switch). Запуск:
   ...
   - ``standard`` — 140 DB-backed тулов без legacy YAML.
   - ``full`` — все 144 тулов, включая legacy. Только для админ-сценариев
