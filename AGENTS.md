...
> + `cod_doc/mcp/tools/curator_tools.py`. Счётчики 6/21/141/145.
...
9. **MCP server profiles** (PCA-951, cycle-4 default-switch). Запуск:
   ...
   - ``standard`` — 141 DB-backed тулов без legacy YAML.
   - ``full`` — все 145 тулов, включая legacy. Только для админ-сценариев
