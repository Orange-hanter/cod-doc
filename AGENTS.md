...
> `cod_doc/mcp/tools/agent_tools.py`. Счётчики 6/21/122/126 пока те же.
...
9. **MCP server profiles** (PCA-951, cycle-4 default-switch). Запуск:
   ...
   - ``standard`` — 122 DB-backed тулов без legacy YAML.
   - ``full`` — все 126 тулов, включая legacy. Только для админ-сценариев
