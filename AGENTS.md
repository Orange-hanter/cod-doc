...
> + `cod_doc/mcp/tools/curator_tools.py`. Счётчики 6/21/138/142.
...
9. **MCP server profiles** (PCA-951, cycle-4 default-switch). Запуск:
   ...
   cod-doc-mcp --profile full                 # все 142, включая legacy
   ...
   - ``agent`` — **default**: 6 curator-тулов (RFC 25 §3.2/§3.5,
     ...
     Вход в работу — `curator_next(project=...)`: дрейф, битые ссылки,
     протухшие хэши MASTER.md и findings одной очередью с готовой
     командой на каждый пункт. Старые task-centric тулы
   ...
   - ``standard`` — 138 DB-backed тулов без legacy YAML.
   - ``full`` — все 142 тулов, включая legacy. Только для админ-сценариев
