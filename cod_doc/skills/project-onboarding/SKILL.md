...
## Разложить импортированное по дереву

Импорт зеркалит файловую кучу: каталоги вместо информационной архитектуры и
`module-spec` на всём подряд. Отдельная фаза после него — `doc tree init` →
`doc tree classify --dry-run` → `--apply` → разбор Инбокса. Правила, роли
разделов и критерии готового дерева — в скилле [doc-structure](../doc-structure/SKILL.md); он же нужен,
если хаос уже в БД и `--dry-run` спасать поздно.
...
Пока хоть один пункт не выполнен — проект заведён наполовину, и curator-цикл
агента (`curator_next` → `ctx_search` / `context_get`) на нём будет отдавать
неполный контекст.
