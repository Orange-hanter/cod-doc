...
```bash
# 1. Реестр: путь + человекочитаемое имя (slug выводится из --name)
cod-doc project add /path/to/repo --name my-app

# 2. Локальное состояние: .cod-doc/state.db + миграции
cod-doc project init my-app

# 3. Разведка: что вообще попадёт в БД
cod-doc import docs my-app --dry-run

# 4. Импорт (по умолчанию cap 1000 файлов)
cod-doc import docs my-app
...
Флага исключений у `import docs` **нет** — единственный рычаг
`--max-files`. Поэтому ручное решение нужно ровно по тем каталогам, которые
walker пропустить не может:
...
Если `--dry-run` показывает такое — **временно вынеси каталог за пределы
repo_root** (или заведи отдельным проектом), импортируй, верни обратно.
Импортировать «а потом почистим» — anti-pattern: удаление документа после
импорта тянет за собой ревизии и ссылки.
...
- **`doc export` на чужом репозитории.** 🔴 Сейчас экспорт **повреждает**
  документ: склеивает преамбулу с первым заголовком, теряет H1, подменяет
  `type` (ADO-010, critical). До закрытия ADO-010 работайте в режиме
  «файлы — источник, БД — индекс»: `import docs` при заведении,
  `doc import <file>` после ручных правок.
