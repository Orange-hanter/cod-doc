...
| `context_depth` | str | `L0` или `L1` — реально загруженный уровень (см. контракт `context_get`). Не возвышай. L2/L3 зарезервированы и сейчас не поддерживаются. |
...
- `context_depth="L2"` / `"L3"` — сейчас невалидно: `context_get`
  декларирует только L0 и L1. Если действительно нужен расширенный
  анализ зависимостей — собирай его последовательностью L1-вызовов и
  отметь в `missing_info`, что L2 отсутствует.
