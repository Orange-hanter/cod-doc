...
`doc_drift` отдаёт `orphan_sections` — якоря секций уровня 2, которые есть в
БД и которых уже нет в файле. Считается **независимо** от четырёх статусов, и
это не педантизм: четвёрка партиционирует состояния хэша контента, а
осиротевшая секция проходит сквозь все четыре. `doc_accept` пиннит хэш файла в
`content_sha256_head`, файл продолжает совпадать с пином — и документ вечно
читается `in_sync`, пока в его теле в БД лежат разделы, выброшенные из файла
месяцы назад. Ровно так `MASTER` проекта Restate жил с девятью секциями против
пяти в `MASTER.md`.

Механизм — тот же, что и у `metadata_mismatch` (ADO-092).

Лечение — `doc import <path> --replace`: секций, которых нет в файле, не
станет, порядок в БД приведётся к порядку в файле. Без `--replace` импорт
остаётся аддитивным (он используется как «подтянуть правки», и тихо потерять
секцию из-за неполного файла хуже), но теперь **называет** сироты в отчёте.
Точечно — `doc_delete_section(project, doc_key, anchor)` / `cod-doc doc
delete-section`; `dry_run` показывает удаляемое тело.

Обратное направление — секция есть в файле, но не в БД — это не сирота, а
неприменённый импорт: обычный `doc import`.
