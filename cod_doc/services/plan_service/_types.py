...
def _pct_closed(total: int, done: int, cancelled: int) -> int:
    ...
    Округление — вниз, а не ``round``: иначе 199 закрытых из 200 давали бы
    «100%» при статусе не DONE — то же расхождение статуса и процента, только
    в обратную сторону (ai-review #87). Вниз — значит 100 ровно тогда, когда
    закрыто всё.
    """
    if total <= 0:
        return 0
