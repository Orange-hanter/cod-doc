...
# ADO-164: форма `\s(?P<want>…)` в обоих паттернах — не стиль, а стоимость
# разбора. Разделитель между «I want» / «я хочу» и телом — ровно один `\s`,
# дальше пробелы забирает само тело (оно всё равно проходит `.strip()`).
# Пока там стоял `\s+`, пробельный прогон могли поделить и квантор, и тело:
# на каждый из k способов деления движок заново сканировал хвост, то есть
# O(n·k). Замер на «Как роль, я хочу » + 8000 пробелов: 215 мс (EN) и 333 мс
# (RU) против 0.06 / 0.08 мс после правки; на 60 000 — больше пяти секунд
# против 0.4 / 0.6 мс. Хендлер синхронный, так что это был отказ в
# обслуживании ровно на одну историю в БД.
#
# То же в русском мосте: `\s*,?\s*` — два пересекающихся квантора вокруг
# необязательной запятой. Заменён на один класс `[\s,]*`, а тело обязано
# закончиться на не-пробел и не-запятую (`.*?[^\s,]`) — тогда мост
# принадлежит мосту, и делить нечего. Английскому мосту это не нужно:
# там запятая обязательна и служит якорем.
#
# Разбор при этом тождественный: проверено на корпусе из 11 формулировок
# (обе формулы, перенос строки, запятая внутри тела, отсутствие запятой
# перед «чтобы», кратные пробелы) — все 11 дают те же группы после `.strip()`.
_NARRATIVE_PATTERNS = (
    ...
    #
    # Необязательны три вещи, и каждая — по живому корпусу, а не из
    # вежливости к грамматике:
    #
    # 1. Префикс «Как <роль>,» — роль хранится отдельной колонкой
    #    `user_story.persona` и рисуется чипом; в прозе её дублируют редко.
    # 2. Местоимение «я». Это не стилистика: в корпусе cod-doc ВСЕ 31
    #    нарратива начинаются прямо с «хочу …». Пока «я» было обязательным,
    #    русский паттерн разбирал 0 из 31 — то есть ADO-144 чинил ровно ту
    #    поломку, ради которой писался, и не чинил. Тесты этого не видели:
    #    все их примеры написаны в полной форме «Как роль, я хочу X, чтобы Y».
    #    Поймалось только показом страницы в браузере.
    # 3. Запятая перед «чтобы», и она же допустима сразу после «хочу»
    #    («хочу, чтобы X, чтобы Y» — US-029).
    #
    # После правки разбирается 29 из 31. Оставшиеся два (US-024, US-025)
    # пользуются «для …» вместо «чтобы» — у них честный fallback в `raw`.
    # Ветку под «для» намеренно не заводим: это предлог, который стоит в
    # середине почти каждого предложения, и угадывание по нему даст мусор
    # вместо разбора.
    re.compile(
        r"^\s*"
        r"(?:\[\s*(?P<header>[^\]]+)\]\s*)?"
        r"(?:Как\s+(?P<role>[^,]+?),\s*)?"
        r"(?:я\s+)?хочу[\s,](?P<want>.*?[^\s,])"
        r"[\s,]*чтобы\s+(?P<so_that>.+)$",
        re.IGNORECASE | re.DOTALL,
    ),
)

# ADO-164, вторая половина: потолок длины входа.
#
# Паттерны выше теперь линейны, так что потолок — не лечение, а страховка:
# `_parse_narrative` зовётся на каждую историю списка и повторно в анализе
# секции, а `story_create` открыт на MCP-поверхности, то есть длину
# `narrative` не ограничивает ничто. Любая будущая правка регекса, вернувшая
# перекрывающиеся кванторы, упрётся в потолок вместо того, чтобы вешать
# синхронный хендлер.
#
# 4096 при живом корпусе в 31 историю с самым длинным нарративом в 302
# символа (avg 181) — тринадцатикратный запас. Выше потолка возвращается тот
# же честный fallback в `raw`, что и при несовпадении паттерна: новых
# состояний UI не появляется.
_MAX_NARRATIVE_PARSE = 4096
...
def _parse_narrative(text: str) -> dict[str, str]:
    ...
    if len(text) > _MAX_NARRATIVE_PARSE:
