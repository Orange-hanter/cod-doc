...
| 25  | [Doc-curator agent](25-doc-curator-agent.md) | 🟢 **Реализован (закрыт 2026-09-20)** | Дефолтный агент = куратор документации и поиска, не исполнитель задач | средний |
...
## 🔧 Graph-editing track (2026-09-22)

> Рождён не из идеи, а из инцидента: пять правок на `Restate` (заголовок и
> позиция секции плана, три ребра зависимости) пришлось вносить SQL-ом мимо
> продукта, потому что штатного интерфейса для них нет ни на одной поверхности.

| #   | Документ                                                | Категория      | Эффект                                            | Риск    |
| --- | ------------------------------------------------------- | -------------- | ------------------------------------------------- | ------- |
| 26  | [Правка графа плана](26-plan-graph-editing.md) | 🔵 Архитектура · **черновик** | Секции и рёбра как первоклассные операции: сервисный слой вместо записи из тула, обязательные `reason`/`note`, гейт «презентация не пишет ORM», рутина `graph_health` | низкий |

## 🤖 Agent-fit track (2026-09-23)

> Рождён из замера: за 40 сессий coding-агент 407 раз читал `state.db`
> SQL-ом в обход тулов — больше, чем сделал MCP-вызовов вообще.

| #   | Документ                                                | Категория      | Эффект                                            | Риск    |
| --- | ------------------------------------------------------- | -------------- | ------------------------------------------------- | ------- |
| 27  | [Agent fit](27-agent-fit.md) | 🟢 Adoption · **черновик** | Выдача по бюджету, роль по профилю, read-тулы вместо SQL, надёжный `task_create`, один канон скиллов; план `agent-fit-2026-09` (AFT-001…016) | низкий |
...
**Doc-curator-track (RFC 25, закрыт 2026-09-20):** дефолтный агент перестал исполнять задачи; роль — документация + поиск. План `doc-curator-2026-09` (18 задач CUR-001…018) закрыт целиком: секция A (политика), секция B (своп профиля + `ctx_search`, CUR-007/008), секция C (качество поиска, кросс-проект SYM-011), секция D (`curator_next` doc card CUR-016, ревизия legacy-daemon CUR-017). RFC 19 не воскрешён целиком — только FTS-агрегатор поверх `search_service.search`.
