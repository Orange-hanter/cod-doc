...
| 25  | [Doc-curator agent](25-doc-curator-agent.md) | 🔵 Архитектура · **секция B закрыта (CUR-007/008)** | Дефолтный агент = куратор документации и поиска, не исполнитель задач | средний |
...
**Doc-curator-track (2026-09-15, RFC 25):** дефолтный агент перестаёт исполнять задачи; роль — документация + поиск. План `doc-curator-2026-09`: секция A (политика, CUR-001…003) и секция B (своп профиля + `ctx_search`, CUR-007/008) закрыты; секция C (качество поиска) и D (`curator_next`/daemon) — в очереди. RFC 19 не воскрешаем целиком — только FTS-агрегатор поверх `search_service.search`.
