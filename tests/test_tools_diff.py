...
async def test_tools_diff_missing_snapshot_returns_hint(tmp_path: Path, monkeypatch) -> None:  # type: ignore[no-untyped-def]
    # Use a name that surely doesn't exist on disk.
    diff = _get_tool("tools_diff")
    result = await diff(since="this-snapshot-does-not-exist-9999")
    assert result["snapshot_found"] is False
    assert "hint" in result


async def test_tools_diff_against_fresh_snapshot_is_empty(tmp_path: Path, monkeypatch) -> None:  # type: ignore[no-untyped-def]
    ...
    from scripts.snapshot_tools import take_snapshot_async

    snap_dir = Path(__file__).resolve().parents[1] / ".cod-doc" / "tool_snapshots"
    snap_dir.mkdir(parents=True, exist_ok=True)
    snap_path = snap_dir / "test-fresh.json"
    try:
        await take_snapshot_async(name="test-fresh", out_dir=snap_dir)
        diff = _get_tool("tools_diff")
        result = await diff(since="test-fresh")
...
async def test_tools_diff_against_synthetic_old_snapshot(tmp_path: Path) -> None:
    ...
    try:
        diff = _get_tool("tools_diff")
        result = await diff(since="test-synthetic")
        assert result["snapshot_found"] is True
