...
def test_agent_get_full_doc_body(engine_with_schema) -> None:  # type: ignore[no-untyped-def]
    ...
    with transactional(factory) as session:
        r = agent_service.get(
            session, project_id=1, task_id="X", what="full_doc_body", ref="guide/x"
        )
...
def test_agent_report_blocker_sets_blocked_reason(engine_with_schema) -> None:  # type: ignore[no-untyped-def]
    ...
    with transactional(factory) as session:
        r = agent_service.report(
            ...
            task_id="REP-002",
            kind="blocker",
            message="waiting on stakeholder",
            agent_id="alpha",
        )
    assert r["ok"] is True
    with transactional(factory) as session:
        row = session.execute(select(TaskModel).where(TaskModel.task_id == "REP-002")).scalar_one()
...
def test_agent_complete_marks_done_and_releases(engine_with_schema) -> None:  # type: ignore[no-untyped-def]
    ...
    with transactional(factory) as session:
        row = session.execute(select(TaskModel).where(TaskModel.task_id == "CMP-001")).scalar_one()
...
def test_agent_release_drops_lock_without_done(engine_with_schema) -> None:  # type: ignore[no-untyped-def]
    ...
    assert r["ok"] is True
    with transactional(factory) as session:
        row = session.execute(select(TaskModel).where(TaskModel.task_id == "REL-001")).scalar_one()
