...
def test_list_filters_by_status_and_type(engine_with_schema) -> None:  # type: ignore[no-untyped-def]
    ...
    with transactional(factory) as session:
        proj_id = _seed_project(session)
        a1 = approvals.request(session, proj_id, approval_type="plan_review", requested_by="x")
        approvals.request(session, proj_id, approval_type="risky_action", requested_by="x")
        approvals.resolve(session, proj_id, a1.approval_id, decision="approve", resolved_by="op")

    with transactional(factory) as session:
        pending = approvals.list_approvals(session, proj_id, status="pending")
        assert pending["total"] == 1
        risky = approvals.list_approvals(session, proj_id, approval_type="risky_action")
...
def test_resolve_returns_no_wake_hint_when_no_linked_tasks(engine_with_schema) -> None:  # type: ignore[no-untyped-def]
    ...
    with transactional(factory) as session:
        proj_id = _seed_project(session)
        a = approvals.request(session, proj_id, approval_type="manual", requested_by="x")
        result = approvals.resolve(
            session, proj_id, a.approval_id, decision="approve", resolved_by="op"
        )
...
def test_resolve_invalid_decision_raises(engine_with_schema) -> None:  # type: ignore[no-untyped-def]
    ...
    with transactional(factory) as session:
        ...
        with pytest.raises(ValueError, match="must be 'approve' or 'deny'"):
            approvals.resolve(session, proj_id, a.approval_id, decision="maybe", resolved_by="op")
...
def test_resolve_already_resolved_raises(engine_with_schema) -> None:  # type: ignore[no-untyped-def]
    ...
    with transactional(factory) as session:
        proj_id = _seed_project(session)
        a = approvals.request(session, proj_id, approval_type="manual", requested_by="x")
        approvals.resolve(session, proj_id, a.approval_id, decision="approve", resolved_by="op")
        with pytest.raises(ValueError, match="already"):
            approvals.resolve(
                session, proj_id, a.approval_id, decision="approve", resolved_by="op2"
            )
...
def test_resolve_unknown_id_raises_lookup_error(engine_with_schema) -> None:  # type: ignore[no-untyped-def]
    ...
    with transactional(factory) as session:
        ...
        with pytest.raises(LookupError):
            approvals.resolve(session, proj_id, "no-such", decision="approve", resolved_by="op")
