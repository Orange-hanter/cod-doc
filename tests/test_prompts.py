...
@pytest.mark.parametrize(
    ...
    [
        "Snowball Protocol",
        "Гибридные ссылки",
        # RFC 25 (CUR-002): секция переименована из «Алгоритм выполнения
        # задачи» — дефолтный агент больше не исполняет продуктовые задачи,
        # цикл теперь drift → контекст → починка документации. Требование
        # «в промпте есть алгоритм» сохранено, изменилось только имя.
        "Алгоритм работы",
        "Fail-Fast",
        "self_check",
    ],
