...
# ── ADO-196: kanban по высоте ───────────────────────────────────────────


def test_kanban_column_body_caps_its_own_height() -> None:
    """Потолок высоты обязан стоять на том же боксе, что несёт `overflow-y`.

    У `<details>` тело колонки лежит не прямым flex-элементом, а внутри
    UA-бокса `::details-content`, которому `min-height: auto` не даёт
    сжаться ниже содержимого. Поэтому `max-height` на самой `.kanban-col`
    не обрезал ничего и скроллера не создавал: на живой доске колонка была
    750px при теле 6601px, и 74 из 82 карточек Blocked рисовались ниже её
    нижнего бордера (TODO — 411 из 419).
    """
    body = _rule(".kanban-col-body")
    assert "max-height" in body, "колонка снова не ограничивает содержимое по высоте"
    assert "overflow-y: auto" in body, "ограничение без скроллера просто прячет карточки"


def test_kanban_card_keeps_its_height_in_a_full_column() -> None:
    """Карточка — flex-элемент колонки, и при определённой высоте она сжимается.

    Вторая половина того же дефекта, невидимая до первой: как только тело
    колонки получает потолок, 419 карточек TODO делят его между собой и
    сплющиваются в пустые рамки по 18px.
    """
    assert "flex-shrink: 0" in _rule(".kanban-card"), (
        "карточки поделят высоту колонки между собой и сплющатся"
    )
