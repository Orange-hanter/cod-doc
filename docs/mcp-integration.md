...
| `curator_next(project, limit?, include_skill_bodies?, skip_links?)` | **Doc card** (CUR-016, AFT-002): `{card{drift{issues, advisory{count, doc_keys}}, links, master, findings, unplaced}, priority[{kind, ref, reason, suggested_action}], navigation{applicable_skills[{name, description}], next_actions, success_criteria}, meta{generated_at, truncated, counts{drift_issues, drift_advisory, …}, not_collected?}}`. `drift.issues` — только не-`in_sync`; расхождения frontmatter и осиротевшие секции у `in_sync`-документов — в `drift.advisory`. Тела скиллов — только при `include_skill_bodies=true`. `skip_links=true` не собирает `links`, и тогда `meta.not_collected = ["links"]`. Порядок очереди: `missing` → `edited_in_place` → `LINK-BROKEN` → hash `BROKEN` → hash `STALE` → `stale_export` → finding. Read-only, идемпотентен. |
