...
- [ ] Каждая задача очереди прошла `task_checkout` → `task_complete` с
      `commit_sha`. *(ADO-217: не выполнено буквально — ADO-040 закрыт с
      `completed_commit = REPLACE_SHA`; остальные четыре с реальным sha)*
...
> Сверка ADO-217 (2026-09-26): спринт закрыт 2026-09-02; открыт один пункт — sha ADO-040.
