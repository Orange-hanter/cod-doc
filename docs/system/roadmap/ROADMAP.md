...
**Состояние на 2026-09-18.** Волны 1 и 2 закрыты. Секция I — 18/58 (станет
19/58 с закрытием ADO-115). Считать прогресс волны только по секции I нельзя:
её задачи лежат в двух секциях сразу — ADO-146, ADO-157 и findings
ADO-159…164 числятся в «Residual debt», хотя сделаны той же волной.

| Волна | PR | Закрыто |
|---|---|---|
| 1 — сломанное и видимое | шесть PR | 15 задач: даты, рендерер, вкладка ADR, переполнения, ссылки Overview |
| 2 — данные есть, UI не показывает | #54, #55, #56, #57, #59, #60 | ADO-146, ADO-157, ADO-143/144/145/140, ADO-159/160/162/164, ADO-115 |

Волна 2 стоила шести PR вместо четырёх: три задачи (ADO-159/160/162) в первом
подходе были закрыты по плану исполнителя, а не по acceptance из БД, и
доделывались отдельно. Вывод записан в AGENTS.md §11 и стоит того, чтобы его
повторить: сверяйся с acceptance задачи, а не с собственным планом.

**Секция I — не только вёрстка, и это надо читать буквально.** Две задачи
волны 2 к вёрстке отношения не имеют вовсе: ADO-115 (шаги прогона не
переживали процесс — контур телеметрии, ADR-015) и связка ADO-143/144/145
(секции историй и флаг acceptance-критерия — доменная модель и три
поверхности). Планируя волну 3, не считайте «секция I» синонимом «CSS».

Открыто из волны 2, сознательно: ADO-161 (гонка `next_position` ждёт переезда
на Postgres — на SQLite закрыть нечем), ADO-186 (горизонтальная полоса на
390px у общего хрома), ADO-187 (два теста краснеют от нагрузки, а не от кода).

Найдено попутно и требует решения владельца: у проекта **orakul нет БД** —
только legacy-YAML, `state.db` отсутствует. Формулировки acceptance вида «на
orakul разбирается заметная доля из 33» в ADO-143/144 оказались непроверяемы;
33 истории orakul живут только в markdown. Бэкфилл
`scripts/backfill_orakul_story_sections.py` лежит в репозитории с баннером
«ONE-SHOT, НЕ ВЫПОЛНЕН» — запускать его пока не на чем.
