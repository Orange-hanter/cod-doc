...
| audit/2026-05-15-tracks-closure-drift | resolved | cod-doc core |
...
> **Формат записи (DOC-LO-5).** Строка таблицы — `| YYYY-MM-DD | <событие> |`.
> Событие: одно предложение в прошедшем времени с **жирным** заголовком-темой,
> ссылками на audit-отчёт/план и затронутыми id (`COD-NNN`, `WEB-NNN`, …). Тот
> же факт для документов со `status: active` дублируется в `revision` — единый
> формат с [revision-history.md §10](standards/revision-history.md). В обычных
> документах тело секции `## Changelog` **генерируется** из `revision` при
> export (править руками нельзя); этот L0-навигатор — исключение, ведётся вручную.
...
- **Языковой стандарт (DOC-LO-4):** проза — на русском; идентификаторы (поля,
  типы, значения `status`, имена сущностей, таблиц, команд, файлов и id задач) —
  на английском, в `code`-обрамлении. Англоязычные термины-понятия (drift,
  checkout, transclusion) допустимы как термины, но не переводятся туда-обратно
  в пределах одного документа — выбирается одна форма.
- Заголовки секций — `## N. Title`; ссылки — markdown-relative из location документа.
- **Restate-ссылки (DOC-LO-3):** упоминания внутренних артефактов исходного ручного
  стека (`~/Git/Restate`) помечаются префиксом «(Restate)», напр. «(Restate)
  `Documentation Graph.md`», чтобы их не путали с сущностями COD-DOC.
