...
[DATA_MODEL §3](../DATA_MODEL.md#3-таблицы) обещает: «полное body документа собирается через view §4.3a», и решение DOC-HI-8 сделало `Section.body` единственным источником. View `document_body` специфицирован в §4.3a, но миграциями Section A не создан. Кроме того, упомянутая в спеке директория `cod_doc/infra/views/` отсутствует.
