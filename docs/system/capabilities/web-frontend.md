...
| `GET /p/{slug}/scenarios` | Список сценариев по возможностям + пробелы по видам | `scenario_service.list_for_project` + `project_coverage` | ✅ | TSC-015 |
| `GET /p/{slug}/scenarios/{scenario_id}` | Карточка сценария: предусловия, шаги, ожидаемый результат, связи | `scenario_service.get` + `list_steps` + `list_links` | ✅ | TSC-015 |
