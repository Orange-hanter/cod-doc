...
а подставляется на этапе рендера/экспорта.

Правила:

- **Хранение.** Transclusion живёт в таблице `link` как обычная ссылка с флагом
  `transclude=true` (а не отдельной копией текста). Источник истины для тела —
  всегда target-документ.
- **Render / export.** При проекции `![[doc:key#anchor]]` разворачивается в тело
  соответствующей секции target'а (с баннером-источником). Циклы (A включает B
  включает A) обрываются на первом повторе с предупреждением `LK-008`.
- **Индексация (ContextService / embeddings).** Индексируется **только источник
  истины** — секция target'а. Источник, который её transclude'ит, **не**
  переиндексирует встроенный текст: иначе один и тот же контент попадёт в индекс
  дважды и исказит similarity. Поэтому для поиска/контекста transclusion
  прозрачен — находится оригинал, не его вставки.
- **Drift.** Поскольку тело не копируется, transclusion не создаёт projection-drift:
  изменение target'а автоматически отражается в источнике при следующем рендере.

Отличие от обычной ссылки: `[[...]]` ведёт к target'у (навигация), `![[...]]`
встраивает его тело (композиция), но обе — ссылки, обе резолвятся через `link`.
