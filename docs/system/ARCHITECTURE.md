...
**Шаги оркестратора под это правило не подпадают, и это не исключение из него, а другой класс записи (ADO-115).** `agent.thinking` / `agent.tool_call` / `agent.tool_result` — телеметрия прогона: у шага нет парной ревизии и нет сущности, которую можно откатить. Правило §11.3 про то, что след мутации не должен разъезжаться с самой мутацией; у шага мутации нет вовсе, разъезжаться нечему.

Отсюда и отличия, намеренные: шаг пишется **своей** транзакцией, а не транзакцией чего-то ещё (долгоживущая пишущая транзакция на SQLite блокирует основного писателя — MCP-демон, — а между шагами оркестратор всё равно стоит на round-trip к LLM), и сбой записи шага **не роняет прогон** (`record_step` глотает ошибку в debug-лог). Это тот же degraded-контракт, что у best-effort строки `agent_run` рядом.
